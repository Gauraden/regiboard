...
static void EthInfo(const std::string &if_name) {
	struct ifconf ifc;
  struct ifreq  ifr[1];
  ...
  int id = 0;
	...
	strcpy(ifr[id].ifr_name, if_name.c_str());
	// Addr
	ioctl(hndl, SIOCGIFADDR, &ifr[id]);
	...
	// Net addr
	int net     = addr & mask;
	printf("\t name: %s; hw: %X:%X:%X:%X:%X:%X; addr: %d.%d.%d.%d; " \
...
static void ListOfEthDevs() {
	/*
  struct ifconf ifconf;
  struct ifreq  iface[10];
  int hndl = socket(PF_INET, SOCK_DGRAM, IPPROTO_IP);
  if (hndl < 1) {
   	printf("socket error: %d\n", hndl);
    return;
  }
 	printf("List of Eth devices:\n");
  ifconf.ifc_len           = sizeof(iface);
  ifconf.ifc_ifcu.ifcu_buf = (caddr_t)iface;
  ioctl(hndl, SIOCGIFCONF, &ifconf);
  int amount = ifconf.ifc_len / sizeof(struct ifreq);
  for (int id = 0; id < amount; ++id) {
    const struct ifreq &kIFace = iface[id];
   	printf("\t* %s\n", kIFace.ifr_name);
  }
  close(hndl);
  */
	...
	int n;
	n = scandir("/sys/class/net/", &namelist, NULL, alphasort);
	if (n < 0) {
		perror("scandir");
		return;
	}
